...
export default function KeyboardHelpOverlay({ aberto, onFechar }) {
    ...
    return (
        ...
        >
            <div
                className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200"
            ...
            >
                ...
                {/* Modo */}
                <div className="flex items-center justify-center gap-3 px-5 py-3 bg-slate-100/50 dark:bg-slate-950/30 text-xs font-mono border-b border-slate-100 dark:border-slate-800">
                    ...
                    <table className="w-full text-sm">
                        <tbody>
                            {ATALHOS.map(({ tecla, descricao }) => (
                                <tr key={tecla} className="border-b border-slate-100 dark:border-slate-800/50 last:border-none">
                                    <td className="py-2.5 pr-3 w-36">
                                        <kbd className="px-2 py-1 rounded-md bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-200 font-mono text-xs border border-slate-200 dark:border-slate-700 shadow-xs font-semibold">
                                            {tecla}
                                        </kbd>
                                    </td>
                ...
                <p className="px-5 py-3 bg-slate-50/50 dark:bg-slate-950/50 text-center text-xs text-slate-400 border-t border-slate-100 dark:border-slate-800">
                    Pressione <kbd className="px-1 py-0.5 rounded bg-slate-200 dark:bg-slate-800 font-mono text-[11px]">T</kbd> ou <kbd className="px-1 py-0.5 rounded bg-slate-200 dark:bg-slate-800 font-mono text-[11px]">Esc</kbd> para fechar
                </p>
            </div>
        </div>
    );
